...
int get_fluid_params(double X[NDIM], struct GRMHD *modvar) {
    ...
    double del[NDIM];
    double rho, uu;
    double Bp[NDIM], V_u[NDIM], V_h[NDIM],lor,Vfac, VdotV, UdotBp;
    ...
    // if (i==0 || j==0 || k==0 || i==N1-2 || j==N2-2 || k==N3) {
    //     (*modvar).n_e = 0.;
    //     return 0;
    // }//on boundary return 0
    ...
    lor=1./sqrt(1.-V_u[1]*V_u[1]-V_u[2]*V_u[2]-V_u[3]*V_u[3]);
    ...
    // i=ti,j=tj,k=tk;
    // //printf("%d %d %d\n",i,j,k);
    // if(i>200 && i<300 && j==250 && k>400 ){
    //     fprintf(stderr,"%d %d %d\n",i,j,k);
    //     fprintf(stderr,"beta=%g sigma=%g thetae=%g thetae_unit=%g trat=%g\n",
    //            (*modvar).beta,(*modvar).sigma,(*modvar).theta_e,Thetae_unit,trat);
    //     fprintf(stderr,"\n %e %e %e %e %e %e %e %e\n", p[KRHO][i][j][k],
...
void set_units(double M_unit_) {
    ...
    fprintf(stderr, "\nUNITS\n");
    fprintf(stderr, "L_unit,T_unit,M_unit: %g cm %g s %g g\n", L_unit, T_unit,M_unit_);
    fprintf(stderr, "RHO_unit,U_unit,B_unit: %g g/cm^3 %g g/(cm s^2) %g Gauss\n",RHO_unit,U_unit,B_unit);
    fprintf(stderr, "Ne: %g \n", Ne_unit);
}
