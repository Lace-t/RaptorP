...
int num_blocks,num_blocks2, tot_blocks;
...
void init_camera(struct Camera **intensityfield) {
    ...
    num_blocks = IMG_WIDTH / num_pixels_1d;
    num_blocks2 = IMG_HEIGHT / num_pixels_1d;
//    if (num_blocks2 != num_blocks) {
//        fprintf(stderr, "should be squared. EXITING\n");//XF:optimize code for rectangle image
//        exit(1);
//    }
    ...
    for (int block = 0; block < tot_blocks; block++) {
        ...
        x = (int)block / (double)num_blocks2;//XF:optimize code for rectangle image
        y = block % num_blocks2;
...
void get_impact_params(struct Camera **intensityfield, int block) {
    ...
    BLOCK_SIZE_Y = CAM_SIZE_Y / (pow(2, (*intensityfield)[block].level - 1) *
                                 (double)(num_blocks2));//XF:optimize code for rectangle image
