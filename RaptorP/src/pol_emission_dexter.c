...
double get_efficiency(double beta, double sigma){
    double eff;
    if ((sigma > 0.01) && beta < 1. / 4. * sigma)
    {
    double e0 = 1.;
    double e1 = - 0.23;
    double e2 = 0.5;
...
double get_kappa(double beta, double sigma){
    ...
    // kappa function from Meringolo et al. 2023
    if ((sigma > 0.01) && beta < 1. / 4. * sigma)
    {
...
double a_I(double theta_e, double n_e, double nu, double B, double theta_B,
           double j_I_thermal, double beta, double sigma) {
#if (DF == KAPPA)
    ...
    return ((1. - eff) * a_I_thermal(theta_e, n_e, nu, B, theta_B, j_I_thermal) + eff * aI_kappa);
#elif (DF == POWER)
...
double a_Q(double theta_e, double n_e, double nu, double B, double theta_B,
           double j_Q_thermal, double beta, double sigma) {
#if (DF == KAPPA)
    ...
    return ((1. - eff) * a_Q_thermal(theta_e, n_e, nu, B, theta_B, j_Q_thermal) + eff * aQ_kappa);
#elif (DF == POWER)
...
double a_V(double theta_e, double n_e, double nu, double B, double theta_B,
           double j_V_thermal, double beta, double sigma) {
#if (DF == KAPPA)
    ...
    return ((1. - eff) * a_V_thermal(theta_e, n_e, nu, B, theta_B, j_V_thermal) + eff * aV_kappa);
#elif (DF == POWER)
