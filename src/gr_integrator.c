...
double clip(double x){
    if (x<STEPSIZE_MIN) return STEPSIZE_MIN;
    else if (x>STEPSIZE_MAX) return STEPSIZE_MAX;
    else return x;
}
...
//#if (metric == CKS)
//    //double r = sqrt(X_u[1]*X_u[1]+X_u[2]*X_u[2]+X_u[3]*X_u[3]);
//    //return -(r) / (idlx1 + idlx2 + idlx3);
//    double r = sqrt(X_u[1]*X_u[1]+X_u[2]*X_u[2]);
//    if (r<RT_OUTER_CUTOFF) return -MIN(STEPSIZE_MAX,(r) / (idlx1 + idlx2));//XF: Specially for jet
//    else return -(r) / (idlx1 + idlx2);
//#else
//
//
//#if(metric==MKSBHAC || metric == LQMKS)
//
