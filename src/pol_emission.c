...
static double safe_bessel_ratio_12(double x) {
    double k1 = bessel_appr(1, x);
    double k2 = bessel_appr(2, x);

    if (isfinite(k1) && isfinite(k2) && fabs(k2) > 0.)
        return k1 / k2;

    if (x > 50.)
        return (1. + 3. / (8. * x)) / (1. + 15. / (8. * x));

    return 0.;
}

static double safe_bessel_ratio_02(double x) {
    double k0 = bessel_appr(0, x);
    double k2 = bessel_appr(2, x);

    if (isfinite(k0) && isfinite(k2) && fabs(k2) > 0.)
        return k0 / k2;

    if (x > 50.)
        return (1. - 1. / (8. * x)) / (1. + 15. / (8. * x));
...
double rho_Q_thermal(double theta_e, double n_e, double nu, double B,
    ...
    return - n_e*ELECTRON_CHARGE*ELECTRON_CHARGE*nu_c*nu_c*sin(theta_B) * sin(theta_B)/\
            (ELECTRON_MASS*SPEED_OF_LIGHT*nu*nu*nu)*f_m(X)*(safe_bessel_ratio_12(Thetaer) + 6. * theta_e);
}
...
double rho_V_kappa(double theta_e, double n_e, double nu, double B,
    ...
    return 2 * n_e * pow(ELECTRON_CHARGE, 2.) * nuc * cos(theta_B) /
           ...
           safe_bessel_ratio_02(1. / w) * factor_G *
           factor_V;
}
...
double rho_V_thermal(double theta_e, double n_e, double nu, double B,
    ...
    fit_factor = (k0 - DeltaJ_5(X)) / k2;
#else
    double shgmfunc = 1 - 0.11*log(1 + 0.035*X);
    double k_ratio = safe_bessel_ratio_02(Thetaer);

    fit_factor = k_ratio * shgmfunc;
#endif

    return 2.0 * n_e *ELECTRON_CHARGE*ELECTRON_CHARGE*nu_c /
           (ELECTRON_MASS*SPEED_OF_LIGHT*nu*nu) * fit_factor * cos(theta_B);
}
