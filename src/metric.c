...
void metric_dd(double X_u[4], double g_dd[4][4]) {
    ...
    double sth2 = sth * sth;
    double rho2 = r * r + a * a * cth * cth;//rho2=r*r
    double z = 2. * r / rho2;//z=2/r
    double delta = r * r - 2. * r + a * a;//delta =r * r - 2. * r
    double sigma2 =(r * r + a * a) * (r * r + a * a) - a * a * delta * sth * sth;// sigma2=r^4
    double omega = sqrt(sigma2) * sth / sqrt(rho2);//omega=r*sth
    ...
    double costh = cos(theta);
    double tfac, rfac, hfac, pfac;
    double rho2 = r * r + a * a * costh * costh;//rho2=r^2
    ...
    double costh = cos(theta);
    double tfac, rfac, hfac, pfac;
    double G_LQG = pow((1. + P), 2.) / pow((1. - P), 2.);
    double rp = 2. * G_LQG / (1. + P) / (1. + P);
    double rm = 2. * G_LQG * P*P / (1. + P) / (1. + P);
    double rs = 2. * G_LQG * P / (1. + P) / (1. + P);
    double a0 = Amin / 8. / M_PI;
    double k2 = (pow(r, 4.) + a0*a0) / (r + rs) / (r + rs);
    double rho2 = k2 + a * a * costh * costh;
    double A1 = (k2 + a*a)/(r*r + a*a);
    double delta = (r - rp)*(r - rm)*r*r/(r + rs)/(r + rs) + a*a;
    ...
    g_dd[0][0] = (- (delta - a*a*sin2th)/rho2) * tfac * tfac;
    g_dd[0][1] = (1./rho2*(A1*(2.*G_LQG*r + a*a*sin2th) - a*a*sin2th)) * tfac * rfac;
    g_dd[0][3] = (-(a*sin2th*(k2 + a*a - delta)) / rho2) * tfac * pfac;

    g_dd[1][0] = g_dd[0][1];
    g_dd[1][1] = (A1/rho2*(A1*(delta + 4.*G_LQG*r + a*a*sin2th) - 2*a*a*sin2th)) * rfac * rfac;
    g_dd[1][3] = (-a * sin2th/rho2*((r*r + a*a)*A1*A1 + 2*G_LQG*r*A1 - a*a*sin2th)) * rfac * pfac;

    g_dd[2][2] = rho2 * hfac * hfac;

    g_dd[3][0] = g_dd[0][3];
    g_dd[3][1] = g_dd[1][3];
    g_dd[3][3] = (-sin2th*((delta*a*a*sin2th - pow((k2 + a*a), 2.)))/rho2) * pfac * pfac;
    ...
    double a2 = a * a;
    double r2 =(R2 - a2 + sqrt((R2 - a2) * (R2 - a2) + 4. * a2 * X_u[3] * X_u[3])) *0.5;//r2=R2
    double r = sqrt(r2);

    double sig = (r2 * r2 + a2 * X_u[3] * X_u[3]);//sig=r2*r2
    double del = (r2 + a2);//del=r2
    double isig, idel;
    ...
    if (r < 1)
        ...
        isig = 1 / sig;
    idel = 1 / (del);//idel=1/r2

    double f = 2. * r2 * r * isig;//f=2/r
    double l[4]={0.,0.,0.,0.};//XF:for approximating CAR

//    l[0] = 1;
//    l[1] = (r * X_u[1] + a * X_u[2]) * idel;//l[1]=x/r
//    l[2] = (r * X_u[2] - a * X_u[1]) * idel;//l[2]=y/r
//    l[3] = (X_u[3]) / (r + 1e-3);//l[3]=z/r
...
void metric_uu(double X_u[4], double g_uu[4][4]) {
    ...
    double sigma2 =(r * r + a * a) * (r * r + a * a) - a * a * delta * sth * sth;//sigma2=r^4
    double omega = sqrt(sigma2) * sth / sqrt(rho2);//omega=r*sth
    double ztilde = omega * omega - (z + 1.) * a * a * sth2 * sth2;//ztilde=(r*sth)^2
    ...
    double irho2 = 1. / (r * r + a * a * costh * costh);// irho2=1/r^2

    double hfac = 1. + hslope * cos(2. * X_u[2]);// hfac=1
    ...
    g_uu[0][0] = (-16.*(a2 + r2)*(a2 + r2)*pow((r + rs),8.)*\
                 (-pow(((a*(r4 + a2*((r + rs)*(r + rs)))*(a2*((r + rs)*(r + rs)) + r*(r3 + 2*G_LQG*((r + rs)*(r + rs))))*\
                sinth)/((a2 + r2)*pow((r + rs),4)) - a3*sinth*sin2th),2.) - \
                 ((a2 + r4/((r + rs)*(r + rs)))*(-pow((a2 + r4/((r + rs)*(r + rs))),2) + a2*sin2th*delta)*\
                (-2*a2*sin2th + ((a2 + r4/((r + rs)*(r + rs)))*\
                 (4*G_LQG*r + a2*sin2th + delta))/(a2 + r2)))/(a2 + r2))*rho2)\
                /(pow((2*r4 + a2*((r + rs)*(r + rs)) + a2*((r + rs)*(r + rs))*cos(2*theta)),2.)*\
                 pow((a4*r2 - 4*a2*G_LQG*r3 + a2*r4 - 4*G_LQG*r3*r2 + 2*a4*r*rs - \
                8*a2*G_LQG*r2*rs + 2*a2*r3*rs + a4*rs2 - 4*a2*G_LQG*r*rs2 + \
                a2*r2*rs2 - a2*(a2 + r2)*((r + rs)*(r + rs))*cos(2*theta) - \
                2*(r4 + a2*((r + rs)*(r + rs)))*delta),2.));

    g_uu[0][1] =  ((a2 + r2)*pow((r + rs),4.)*(a02 + r4 + a2*pow((r + rs),2.))*\
                (2.*a02 + a2*r2 + 2.*r4 + 2.*a2*r*rs + a2*rs2 +\
                  a2*pow((r + rs),2.)*cos(2.*theta))*\
                (4.*a02*G_LQG + 4.*a2*G_LQG*r2 + 4.*G_LQG*r4 - a2*r2*rm - a2*r2*rp +\
                  a2*r*rm*rp + 8.*a2*G_LQG*r*rs - 2.*a2*r2*rs + 4.*a2*G_LQG*rs2 - \
                  a2*r*rs2 + a2*r*(-(rm*rp) + rs2 + r*(rm + rp + 2.*rs))*cos(2.*theta)))/\
                 (4.*(a02 + a2*r2 + r4 + 2.*a2*r*rs + a2*rs2 - \
                  a2*pow((r + rs),2.)*sin2th)*\
                pow(((a02 + r4 + a2*pow((r + rs),2.))*\
                    (a2*pow((r + rs),2.) + r*(r*(r - rm)*(r - rp) + 2.*G_LQG*pow((r + rs),2.))) - \
                   a2*(a2 + r2)*pow((r + rs),4.)*sin2th),2.));

    g_uu[1][0] = g_uu[0][1];
    g_uu[1][1] = (((a2 + r2)*(a2 + r2))*pow((r + rs),4.)*(r2*(r - rm)*(r - rp) + a2*((r + rs)*(r + rs)))*\
                (a02 + r4 + a2*((r + rs)*(r + rs))*cos2th))/\
                (r2*pow(((a02 + r4 + a2*((r + rs)*(r + rs)))*\
                (a2*((r + rs)*(r + rs)) + r*(r*(r - rm)*(r - rp) + 2.*G_LQG*((r + rs)*(r + rs)))) - \
                 a2*(a2 + r2)*pow((r + rs),4.)*sin2th),2.));

    g_uu[1][3] = (a*(a2 + r2)*pow((r + rs),4.)*(a02 + r4 + a2*((r + rs)*(r + rs))*cos2th))/\
                (r*(a02 + a2*r2 + r4 + 2.*a2*r*rs + a2*rs2 - a2*((r + rs)*(r + rs))*sin2th)*\
                ((a02 + r4 + a2*((r + rs)*(r + rs)))*\
                 (a2*((r + rs)*(r + rs)) + r*(r*(r - rm)*(r - rp) + 2.*G_LQG*((r + rs)*(r + rs)))) - \
                a2*(a2 + r2)*pow((r + rs),4.)*sin2th));

    g_uu[0][3] = -((a*((r + rs)*(r + rs))*(a02 + r4 + a2*((r + rs)*(r + rs))*cos2th)*\
                ((a02 + r4 + a2*((r + rs)*(r + rs)))*\
                 (a02 + r*(r*(-(rm*rp) + r*(rm + rp)) - 2.*G_LQG*((r + rs)*(r + rs)))) + \
                a2*((r + rs)*(r + rs))*(-a02 + r2*rs*(2.*r + rs))*sin2th))/\
                (pow((a02 + a2*r2 + r4 + 2.*a2*r*rs + a2*rs2 - \
                 a2*((r + rs)*(r + rs))*sin2th),2.)*\
                ((a02 + r4 + a2*((r + rs)*(r + rs)))*\
                 (a2*((r + rs)*(r + rs)) + r*(r*(r - rm)*(r - rp) + 2.*G_LQG*((r + rs)*(r + rs)))) - \
                a2*(a2 + r2)*pow((r + rs),4.)*sin2th)));
    ...
    g_uu[2][2] = 1./(hfac*hfac*((a02 + r4)/((r + rs)*(r + rs)) + a2*cos2th));

    g_uu[3][1] = g_uu[1][3];
    g_uu[3][3] = (2.*((r + rs)*(r + rs))*(1/sin2th))/\
                 (2.*a02 + a2*r2 + 2.*r4 + 2.*a2*r*rs + a2*rs2 + a2*((r + rs)*(r + rs))*cos(2.*theta));
...
// Returns the contravariant metric g_uu at location X
void metric_KS_uu(double X_u[4], double g_uu[4][4]) {// XF: reduce it to Sphere
    // Initialize: set all elements to 0

    LOOP_ij g_uu[i][j] = 0.;
    //XF: I need to redefine it.
    ...
    double irho2 = 1. / (r * r + a * a * costh * costh);//irho2=1./(r*r)
...
void gcov_func(double Xu[4], double gcov[4][4]){//XF:uesless

    double r = R0 + exp(Xu[1]);
    double theta = Xu[2] + 0.5 * hslope * sin(2. * Xu[2]);
    double tfac, rfac, hfac, pfac;
    ...
    tfac = 1.;
    rfac = r;
    hfac = 1. + hslope * cos(2. * Xu[2]);
    pfac = 1.;

    gcov[0][0] = (- (delta - a*a*sin2th)/rho2) * tfac * tfac;
    gcov[0][1] = (1./rho2*(A1*(2.*G_LQG*r + a*a*sin2th) - a*a*sin2th)) * tfac * rfac;
    gcov[0][3] = (-(a*sin2th*(k2 + a*a - delta)) / rho2) * tfac * pfac;

    gcov[1][0] = gcov[0][1];
    gcov[1][1] = (A1/rho2*(A1*(delta + 4.*G_LQG*r + a*a*sin2th) - 2*a*a*sin2th)) * rfac * rfac;
    gcov[1][3] = (-a * sin2th/rho2*((r*r + a*a)*A1*A1 + 2*G_LQG*r*A1 - a*a*sin2th)) * rfac * pfac;

    gcov[2][2] = rho2 * hfac * hfac;

    gcov[3][0] = gcov[0][3];
    gcov[3][1] = gcov[1][3];
    gcov[3][3] = (-sin2th*((delta*a*a*sin2th - pow((k2 + a*a), 2.)))/rho2) * pfac * pfac;

}

void get_connection(double X[4], double conn[4][4][4]){//XF: useless
    int i, j, k, l;
    double tmp[4][4][4];
    double Xh[4], Xl[4];
    double gcon[4][4];
    ...
    pfac = 1.;
     ...
     gcon[0][0] = (-16.*(a2 + r2)*(a2 + r2)*pow((r + rs),8.)*\
                 (-pow(((a*(r4 + a2*((r + rs)*(r + rs)))*(a2*((r + rs)*(r + rs)) + r*(r3 + 2*G_LQG*((r + rs)*(r + rs))))*\
                sinth)/((a2 + r2)*pow((r + rs),4)) - a3*sinth*sin2th),2.) - \
                 ((a2 + r4/((r + rs)*(r + rs)))*(-pow((a2 + r4/((r + rs)*(r + rs))),2) + a2*sin2th*delta)*\
                (-2*a2*sin2th + ((a2 + r4/((r + rs)*(r + rs)))*\
    ...
    gcon[0][1] =  ((a2 + r2)*pow((r + rs),4.)*(a02 + r4 + a2*pow((r + rs),2.))*\
                (2.*a02 + a2*r2 + 2.*r4 + 2.*a2*r*rs + a2*rs2 +\
                  a2*pow((r + rs),2.)*cos(2.*theta))*\
                (4.*a02*G_LQG + 4.*a2*G_LQG*r2 + 4.*G_LQG*r4 - a2*r2*rm - a2*r2*rp +\
                  a2*r*rm*rp + 8.*a2*G_LQG*r*rs - 2.*a2*r2*rs + 4.*a2*G_LQG*rs2 - \
    ...
    gcon[1][0] = gcon[0][1];
    gcon[1][1] = (((a2 + r2)*(a2 + r2))*pow((r + rs),4.)*(r2*(r - rm)*(r - rp) + a2*((r + rs)*(r + rs)))*\
                (a02 + r4 + a2*((r + rs)*(r + rs))*cos2th))/\
                (r2*pow(((a02 + r4 + a2*((r + rs)*(r + rs)))*\
                (a2*((r + rs)*(r + rs)) + r*(r*(r - rm)*(r - rp) + 2.*G_LQG*((r + rs)*(r + rs)))) - \
                 a2*(a2 + r2)*pow((r + rs),4.)*sin2th),2.));

    gcon[1][3] = (a*(a2 + r2)*pow((r + rs),4.)*(a02 + r4 + a2*((r + rs)*(r + rs))*cos2th))/\
                (r*(a02 + a2*r2 + r4 + 2.*a2*r*rs + a2*rs2 - a2*((r + rs)*(r + rs))*sin2th)*\
                ((a02 + r4 + a2*((r + rs)*(r + rs)))*\
                 (a2*((r + rs)*(r + rs)) + r*(r*(r - rm)*(r - rp) + 2.*G_LQG*((r + rs)*(r + rs)))) - \
                a2*(a2 + r2)*pow((r + rs),4.)*sin2th));

    gcon[0][3] = -((a*((r + rs)*(r + rs))*(a02 + r4 + a2*((r + rs)*(r + rs))*cos2th)*\
                ((a02 + r4 + a2*((r + rs)*(r + rs)))*\
                 (a02 + r*(r*(-(rm*rp) + r*(rm + rp)) - 2.*G_LQG*((r + rs)*(r + rs)))) + \
                a2*((r + rs)*(r + rs))*(-a02 + r2*rs*(2.*r + rs))*sin2th))/\
                (pow((a02 + a2*r2 + r4 + 2.*a2*r*rs + a2*rs2 - \
    ...
    gcon[2][2] = 1./(hfac*hfac*((a02 + r4)/((r + rs)*(r + rs)) + a2*cos2th));

    gcon[3][1] = gcon[1][3];
    gcon[3][3] = (2.*((r + rs)*(r + rs))*(1/sin2th))/\
                 (2.*a02 + a2*r2 + 2.*r4 + 2.*a2*r*rs + a2*rs2 + a2*((r + rs)*(r + rs))*cos(2.*theta));
...
#if (metric == CAR)//XF: do nothing for CKS naturally
...
#elif (metric == MKSHARM || metric == MKSBHAC) //  || metric == LQMKS (Modified) Kerr-Schild metric
...
#elif (metric == LQMKS) //  (Modified) Kerr-Schild metric for LQBH

    get_connection(X_u, gamma);
...
void initialize_photon(double alpha, double beta, double photon_u[8],
    ...
    double az = azimuth / 180. * M_PI;
    double Xcam_u[4] = {t_init, logscale ? log(rcam) : rcam, acos(mu0), az};
    double En = 1.;
    double E2 = En * En;
    ...
    double sint = sin(theta);
    double cost = cos(theta);
    double sigma = r * r + a * a * cost * cost;//r^2
    double delta = r * r + a * a - 2. * r;//r*2-2r
    double A_ = (r * r + a * a) * (r * r + a * a) - delta * a * a * sint * sint;//r^4

    /*
    double g_dd_11 = sigma / delta * rfactor * rfactor;//1+2/(r-2)
    double g_uu_00 = -A_ / (sigma * delta);//-1-2/(r-2)
    */
    double g_dd_11 = 1.;//XF: I reduce BL to Spherical
    double g_uu_00 = -1.;

    double g_uu_03 = -2. * a * r / (sigma * delta);//0
    double g_uu_33 =
        (delta - a * a * sint * sint) / (sigma * delta * sint * sint);//1/(r^2*sint^2)
    double g_uu_22 = 1. / sigma;//1/r^2
...
void initialize_photon_perspective(double alpha, double beta,
                                   double photon_u[8], double t_init) {//XF: useless
