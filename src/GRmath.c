...
double get_r(double X_u[4]) {
    ...
    //return sqrt(r2);
    return sqrt(X_u[1] * X_u[1] + X_u[2] * X_u[2]);//XF: specially for jet
#elif (metric == CAR)//XF: for jet or for black hole, it should be crystallized
    //return sqrt(R2);
    return sqrt(X_u[1] * X_u[1] + X_u[2] * X_u[2]);
#else
...
void BL_to_KS_u(double *BLphoton_u, double *KSphoton_u) {
    ...
    double r_current2 = logscale ? exp(BLphoton_u[1]) : BLphoton_u[1];//r
    double delta_current = r_current2 * r_current2 - 2. * r_current2 + a * a;//r^2-2r
    double rfactor = logscale ? r_current2 : 1.;//1
    trans[0][1] = 2. * r_current2 / delta_current * rfactor;//2r/(r^2-2r)
...
void KS_to_CKS(double *X_KS_u, double *X_CKS_u) {//XF: useless
...
void CKS_to_KS(double *X_CKS_u, double *X_KS_u) {
    ...
    double r = sqrt(X_CKS_u[1]*X_CKS_u[1]+X_CKS_u[2]*X_CKS_u[2]+X_CKS_u[3]*X_CKS_u[3]);
...
void KS_to_CKS_u(double *KScoords, double *CKScoords) {//XF：when a=0, it reduces to Sphere to Cartesian
    double trans[4][4];
