...
void output_files(struct Camera *intensityfield,
    ...
    fprintf(specfile,"# freq[Hz] I[Jy] Q[Jy] U[Jy] V[Jy] Luminosity[erg/s]\n");
#endif
    ...
    for (int f = 0; f < num_frequencies; f++) { // For all frequencies...
        ...
        sprintf(hdf5_filename, "%s/img_data_%d_%.02lf.h5", spec_folder,
                (int)TIME_INIT,INCLINATION);
...
#else
        fprintf(specfile, "%+.15e\t%+.15e", frequencies[f],
